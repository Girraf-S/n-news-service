...
public class UserDetailsImplDeserializer extends JsonDeserializer<UserDetailsImpl> {
    @Override
    public UserDetailsImpl deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException, JacksonException {
        JsonNode jsonNode = jsonParser.getCodec().readTree(jsonParser);
        String username = jsonNode.get("username").asText();
        String password = jsonNode.get("password").asText();
        boolean active = jsonNode.get("active").asBoolean();
        Set<SimpleGrantedAuthority> authorities = new HashSet<>();//jsonNode.get("authorities");//.as
